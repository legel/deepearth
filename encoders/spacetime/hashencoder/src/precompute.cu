...
__global__ void kernel_precompute(
    ...
    // Compute scale and resolution for this level. per_level_scale is the LEARNABLE [L, D] log2-resolution array
    // (must match hashencoder.cu's forward: scale = exp2(per_level_scale[level][d]) * base[d]), NOT a scalar growth.
    double scale[D];
...
void hash_encode_precompute(
    ...
    // per_level_scale is ALREADY the learnable [L, D] log2-resolution (the forward does exp2(per_level_scale)); pass it
    // through unchanged. (Do NOT log2() it — that double-log2 collapsed level 0 to exp2(log2(0))=0 -> resolution 1.)
    auto per_level_scale_log2 = per_level_scale;
