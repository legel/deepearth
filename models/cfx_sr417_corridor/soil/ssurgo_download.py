...
======================================================================
Downloads USDA SSURGO soil data for the CFX SR417 corridor test-landscape
AOI (28.36687N, -81.43299W, near Lake Nona / south Orlando, FL) via the
Soil Data Access (SDA) REST API and derives the soil parameters needed
for downstream hydrology/erosion modeling.
...
Usage:
    ...
    python3 soil/ssurgo_download.py --lat 28.36687 --lon -81.43299 --radius_km 1.0
"""
...
def query_ssurgo_spatial(bbox_wsen):
    ...
    if result is None or not result.get("Table"):
        print("  ⚠ SDA spatial query returned no results; using fallback default soil parameters.")
        return None, None
    ...
    # (minimum Ksat = hydraulically limiting layer)
    ...
    if cokey_list:
        ...
        if hz_result and hz_result.get("Table"):
            ...
            # Min Ksat over profile = limiting horizon
...
def build_soil_parameters(components_df, mukeys):
    ...
    for mukey in mukeys:
        ...
        hsg_raw = str(hsg).strip() if hsg is not None else ""
        if not hsg_raw:
            hsg_raw = "B"
        # Dual classifications (e.g. "B/D") mean drained=B, wet=D.
        # Florida spodosols are commonly B/D or A/D — the wet-condition side
        # (D) applies during major storm events with a high water table.
        if "/" in hsg_raw:
            hsg_drained = hsg_raw.split("/")[0].strip().upper()
            hsg_wet     = hsg_raw.split("/")[1].strip().upper()
        else:
            hsg_drained = hsg_raw[0].upper()
            hsg_wet     = hsg_drained   # single designation, same for both conditions
        if hsg_drained not in "ABCD":
            hsg_drained = "B"
        if hsg_wet not in "ABCD":
            hsg_wet = hsg_drained
        hsg = hsg_drained   # keep `hsg` as the drained-condition primary value

        muname   = dom.get("muname", "")
        is_water = "water" in muname.lower()

        if is_water:
            # SSURGO leaves hydgrp blank for open-water map units (there's no soil to
            # rate) — the generic hsg_raw="" fallback above defaults that to "B", which
            # then fed a nonzero Horton infiltration rate to open water. Water doesn't
            # infiltrate; treat it explicitly rather than through the generic fallback.
            horton = {"fc_mm_hr": 0.0, "f0_mm_hr": 0.0, "k_hr": HORTON_K_BY_TEXTURE["default"]}
            cn     = CN_WATER_BODY[hsg]
        else:
        ...
        params[mukey] = {
            ...
            "muname":     muname,
            "hsg":        hsg,
            "hsg_wet":    hsg_wet,    # wet/flood-condition HSG (use for storm simulations)
            "hsg_raw":    hsg_raw,    # original SSURGO string, e.g. "B/D"
            "texture":    texture,
            **horton,
            "cn_residential_quarter_ac": cn,
            "cn_impervious":             98,
            "cn_open_space":             CN_WATER_BODY[hsg] if is_water else CN_OPEN_SPACE_FAIR.get(hsg, 79),
            "cn_open_space_wet":         CN_WATER_BODY[hsg] if is_water else CN_OPEN_SPACE_FAIR.get(hsg_wet, 84),
        }

    return params


def fallback_default_soil_params():
    """
    Generic fallback soil parameters used only if the SDA spatial query fails
    (network issue or zero map units returned). NOT confirmed for this specific
    AOI — the SR417 corridor site has not had its dominant soil series manually
    verified the way Johns Lake's was. These sandy-soil values are a Central
    Florida flatwoods/ridge-region planning-level placeholder pending a real
    SDA response; treat any run that hits this path as needing a follow-up
    check against the actual SSURGO survey for this AOI.
    """
    print("  ⚠ Using generic Central Florida sandy-soil fallback (HSG B) — "
          "NOT confirmed for this AOI. Re-run once SDA is reachable.")
    return {
        "fallback": {
            "mukey": "fallback",
            "muname": "Generic Central FL sandy soil (unconfirmed fallback — not SDA-derived)",
            "hsg": "B",
            "texture": "fine sand",
            "fc_mm_hr":  25.0,
            "f0_mm_hr":  112.5,
...
def main(lat=PROPERTY_LAT, lon=PROPERTY_LON, radius_km=RADIUS_KM, dem_path=None):
    ...
    if components_df is not None and not components_df.empty:
        ...
        params = fallback_default_soil_params()
