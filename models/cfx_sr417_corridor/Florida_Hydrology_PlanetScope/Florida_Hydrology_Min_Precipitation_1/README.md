...
_Scene `20210317_152511_08_2264` · generated 2026-06-26 00:54 UTC by `visualize_psscene.py`_
...
| Property | Value |
|---|---|
| Item ID | `20210317_152511_08_2264` |
| Instrument / satellite | PSB.SD / 2264 |
| Acquired (UTC) | 2021-03-17T15:25:11.088842Z |
...
| Sun elev / azim | 47.5° / 128.3° |
| View angle | 3° |
| Cloud / clear | 0% / 100% |
| Quality category | standard |
| UDM2 clear (in-footprint) | 100.0% |
| UDM2 mean confidence | 99.9% |
| Open water (sub-pixel) | 4.2% (17.0 ha; 79 bodies) |

## 2. Original data & paths

| Role | File |
|---|---|
| Surface-reflectance cube (8-band) | `PSScene/20210317_152511_08_2264_3B_AnalyticMS_SR_8b_clip.tif` |
| UDM2 quality mask (8-band) | `PSScene/20210317_152511_08_2264_3B_udm2_clip.tif` |
| Band radiometry XML | `PSScene/20210317_152511_08_2264_3B_AnalyticMS_8b_metadata_clip.xml` |
| Item metadata (STAC props) | `PSScene/20210317_152511_08_2264_metadata.json` |
...
|---|---|---|---|---|
| 1 | Coastal Blue | 443 | `cividis` | 0.018 – 0.117 |
| 2 | Blue | 490 | `gray` | 0.021 – 0.134 |
| 3 | Green I | 531 | `gray` | 0.031 – 0.158 |
| 4 | Green | 565 | `gray` | 0.032 – 0.168 |
| 5 | Yellow | 610 | `gray` | 0.031 – 0.182 |
| 6 | Red | 665 | `gray` | 0.027 – 0.182 |
| 7 | Red Edge | 705 | `magma` | 0.033 – 0.200 |
| 8 | NIR | 865 | `inferno` | 0.048 – 0.355 |
...
|---|---|---|
| 1 | 2.2212275274258175e-05 | 0.01 |
| 2 | 2.1451749121730013e-05 | 0.01 |
| 3 | 2.2926965656463335e-05 | 0.01 |
| 4 | 2.3266088061217102e-05 | 0.01 |
| 5 | 2.5005582140139823e-05 | 0.01 |
| 6 | 2.8003949127068466e-05 | 0.01 |
| 7 | 2.9937953528476228e-05 | 0.01 |
| 8 | 4.4323210201119507e-05 | 0.01 |
...
| `derived/ndvi.png / ndvi_annotated.png` | NDVI = (NIR-Red)/(NIR+Red) (vegetation vigour / greenness); range [-1,1], `RdYlGn`. Scene median +0.537. |
| `derived/ndre.png / ndre_annotated.png` | NDRE = (NIR-RedEdge)/(NIR+RedEdge) (canopy chlorophyll / N status); range [-1,1], `RdYlGn`. Scene median +0.380. |
| `derived/ndwi.png / ndwi_annotated.png` | NDWI = (Green-NIR)/(Green+NIR) (open water (McFeeters)); range [-1,1], `RdBu`. Scene median -0.537. |
| `derived/ndwire.png / ndwire_annotated.png` | NDWIre = (RedEdge-NIR)/(RedEdge+NIR) (red-edge water variant; separates dark water from shadow/wet vegetation); range [-1,1], `RdBu`. Scene median -0.380. |
...
- **RGB / false-colour exposure:** independent per-channel percentile stretch at **2–98%** of valid pixels (a gray-world white balance), then **gamma 1.8**. Per-channel reflectance windows used: Red 0.027–0.182, Green 0.032–0.168, Blue 0.021–0.134.
...
```bash
python visualize_psscene.py Florida_Hydrology_Min_Precipitation_1 \
...
python visualize_psscene.py Florida_Hydrology_Min_Precipitation_1 --s2-fusion
```
