...
_Scene `20250529_163323_20_2531` · generated 2026-06-26 00:47 UTC by `visualize_psscene.py`_
...
| Property | Value |
|---|---|
| Item ID | `20250529_163323_20_2531` |
| Instrument / satellite | PSB.SD / 2531 |
| Acquired (UTC) | 2025-05-29T16:33:23.200527Z |
...
| Sun elev / azim | 76.8° / 117.3° |
| View angle | 2.7° |
| Cloud / clear | 7% / 0% |
| Quality category | standard |
| UDM2 clear (in-footprint) | 0.1% |
| UDM2 mean confidence | 87.1% |
| Open water (sub-pixel) | 0.9% (3.8 ha; 43 bodies) |

## 2. Original data & paths

| Role | File |
|---|---|
| Surface-reflectance cube (8-band) | `PSScene/20250529_163323_20_2531_3B_AnalyticMS_SR_8b_clip.tif` |
| UDM2 quality mask (8-band) | `PSScene/20250529_163323_20_2531_3B_udm2_clip.tif` |
| Band radiometry XML | `PSScene/20250529_163323_20_2531_3B_AnalyticMS_8b_metadata_clip.xml` |
| Item metadata (STAC props) | `PSScene/20250529_163323_20_2531_metadata.json` |
...
|---|---|---|---|---|
| 1 | Coastal Blue | 443 | `cividis` | 0.116 – 0.259 |
| 2 | Blue | 490 | `gray` | 0.111 – 0.259 |
| 3 | Green I | 531 | `gray` | 0.122 – 0.274 |
| 4 | Green | 565 | `gray` | 0.125 – 0.281 |
| 5 | Yellow | 610 | `gray` | 0.121 – 0.284 |
| 6 | Red | 665 | `gray` | 0.114 – 0.282 |
| 7 | Red Edge | 705 | `magma` | 0.140 – 0.304 |
| 8 | NIR | 865 | `inferno` | 0.218 – 0.424 |
...
|---|---|---|
| 1 | 1.743628015794028e-05 | 0.01 |
| 2 | 1.683927931497444e-05 | 0.01 |
| 3 | 1.799730065568084e-05 | 0.01 |
| 4 | 1.826350630927157e-05 | 0.01 |
| 5 | 1.9628981287349327e-05 | 0.01 |
| 6 | 2.1982651325870547e-05 | 0.01 |
| 7 | 2.3500813790240665e-05 | 0.01 |
| 8 | 3.479300976706479e-05 | 0.01 |
...
| `derived/ndvi.png / ndvi_annotated.png` | NDVI = (NIR-Red)/(NIR+Red) (vegetation vigour / greenness); range [-1,1], `RdYlGn`. Scene median +0.349. |
| `derived/ndre.png / ndre_annotated.png` | NDRE = (NIR-RedEdge)/(NIR+RedEdge) (canopy chlorophyll / N status); range [-1,1], `RdYlGn`. Scene median +0.250. |
| `derived/ndwi.png / ndwi_annotated.png` | NDWI = (Green-NIR)/(Green+NIR) (open water (McFeeters)); range [-1,1], `RdBu`. Scene median -0.327. |
| `derived/ndwire.png / ndwire_annotated.png` | NDWIre = (RedEdge-NIR)/(RedEdge+NIR) (red-edge water variant; separates dark water from shadow/wet vegetation); range [-1,1], `RdBu`. Scene median -0.250. |
...
- **RGB / false-colour exposure:** independent per-channel percentile stretch at **2–98%** of valid pixels (a gray-world white balance), then **gamma 1.8**. Per-channel reflectance windows used: Red 0.114–0.282, Green 0.125–0.281, Blue 0.111–0.259.
...
```bash
python visualize_psscene.py Florida_Hydrology_Max_Precipitation_2 \
...
python visualize_psscene.py Florida_Hydrology_Max_Precipitation_2 --s2-fusion
```
