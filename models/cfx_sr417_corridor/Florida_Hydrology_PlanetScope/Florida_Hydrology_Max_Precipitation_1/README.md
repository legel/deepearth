...
_Scene `20220930_152620_13_2276` · generated 2026-06-26 00:54 UTC by `visualize_psscene.py`_
...
| Property | Value |
|---|---|
| Item ID | `20220930_152620_13_2276` |
| Instrument / satellite | PSB.SD / 2276 |
| Acquired (UTC) | 2022-09-30T15:26:20.134759Z |
| Ground sample distance | 3.9 m (pixel grid 3 m) |
| Raster size | 670 × 670 px, 8 bands, uint16 |
| CRS | EPSG:32617 |
| Bounds (CRS units) | (456567.0, 3136929.0, 458577.0, 3138939.0) |
| Sun elev / azim | 49.4° / 135.9° |
| View angle | 3° |
| Cloud / clear | 0% / 100% |
| Quality category | standard |
| UDM2 clear (in-footprint) | 100.0% |
| UDM2 mean confidence | 98.6% |
| Open water (sub-pixel) | 6.5% (25.9 ha; 106 bodies) |

## 2. Original data & paths

| Role | File |
|---|---|
| Surface-reflectance cube (8-band) | `PSScene/20220930_152620_13_2276_3B_AnalyticMS_SR_8b_clip.tif` |
| UDM2 quality mask (8-band) | `PSScene/20220930_152620_13_2276_3B_udm2_clip.tif` |
| Band radiometry XML | `PSScene/20220930_152620_13_2276_3B_AnalyticMS_8b_metadata_clip.xml` |
| Item metadata (STAC props) | `PSScene/20220930_152620_13_2276_metadata.json` |
...
|---|---|---|---|---|
| 1 | Coastal Blue | 443 | `cividis` | 0.018 – 0.156 |
| 2 | Blue | 490 | `gray` | 0.018 – 0.170 |
| 3 | Green I | 531 | `gray` | 0.022 – 0.186 |
| 4 | Green | 565 | `gray` | 0.027 – 0.207 |
| 5 | Yellow | 610 | `gray` | 0.024 – 0.224 |
| 6 | Red | 665 | `gray` | 0.021 – 0.225 |
| 7 | Red Edge | 705 | `magma` | 0.027 – 0.242 |
| 8 | NIR | 865 | `inferno` | 0.039 – 0.390 |
...
|---|---|---|
| 1 | 2.1820132540402628e-05 | 0.01 |
| 2 | 2.1073032963987842e-05 | 0.01 |
| 3 | 2.252220554609513e-05 | 0.01 |
| 4 | 2.285534097359106e-05 | 0.01 |
| 5 | 2.4564125458147063e-05 | 0.01 |
| 6 | 2.7509558298850757e-05 | 0.01 |
| 7 | 2.9409419157379974e-05 | 0.01 |
| 8 | 4.3540713828869705e-05 | 0.01 |
...
| `derived/ndvi.png / ndvi_annotated.png` | NDVI = (NIR-Red)/(NIR+Red) (vegetation vigour / greenness); range [-1,1], `RdYlGn`. Scene median +0.529. |
| `derived/ndre.png / ndre_annotated.png` | NDRE = (NIR-RedEdge)/(NIR+RedEdge) (canopy chlorophyll / N status); range [-1,1], `RdYlGn`. Scene median +0.361. |
| `derived/ndwi.png / ndwi_annotated.png` | NDWI = (Green-NIR)/(Green+NIR) (open water (McFeeters)); range [-1,1], `RdBu`. Scene median -0.497. |
| `derived/ndwire.png / ndwire_annotated.png` | NDWIre = (RedEdge-NIR)/(RedEdge+NIR) (red-edge water variant; separates dark water from shadow/wet vegetation); range [-1,1], `RdBu`. Scene median -0.361. |
...
- **RGB / false-colour exposure:** independent per-channel percentile stretch at **2–98%** of valid pixels (a gray-world white balance), then **gamma 1.8**. Per-channel reflectance windows used: Red 0.021–0.225, Green 0.027–0.207, Blue 0.018–0.170.
...
```bash
python visualize_psscene.py Florida_Hydrology_Max_Precipitation_1 \
...
python visualize_psscene.py Florida_Hydrology_Max_Precipitation_1 --s2-fusion
```
