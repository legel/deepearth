...
_Scene `20220419_154347_41_2480` · generated 2026-06-26 00:47 UTC by `visualize_psscene.py`_
...
| Property | Value |
|---|---|
| Item ID | `20220419_154347_41_2480` |
| Instrument / satellite | PSB.SD / 2480 |
| Acquired (UTC) | 2022-04-19T15:43:47.417883Z |
| Ground sample distance | 4.1 m (pixel grid 3 m) |
| Raster size | 670 × 670 px, 8 bands, uint16 |
| CRS | EPSG:32617 |
| Bounds (CRS units) | (456567.0, 3136929.0, 458577.0, 3138939.0) |
| Sun elev / azim | 60.5° / 120.2° |
| View angle | 4° |
| Cloud / clear | 0% / 100% |
| Quality category | standard |
| UDM2 clear (in-footprint) | 100.0% |
| UDM2 mean confidence | 99.7% |
| Open water (sub-pixel) | 3.9% (15.5 ha; 40 bodies) |

## 2. Original data & paths

| Role | File |
|---|---|
| Surface-reflectance cube (8-band) | `PSScene/20220419_154347_41_2480_3B_AnalyticMS_SR_8b_clip.tif` |
| UDM2 quality mask (8-band) | `PSScene/20220419_154347_41_2480_3B_udm2_clip.tif` |
| Band radiometry XML | `PSScene/20220419_154347_41_2480_3B_AnalyticMS_8b_metadata_clip.xml` |
| Item metadata (STAC props) | `PSScene/20220419_154347_41_2480_metadata.json` |
...
|---|---|---|---|---|
| 1 | Coastal Blue | 443 | `cividis` | 0.024 – 0.155 |
| 2 | Blue | 490 | `gray` | 0.026 – 0.181 |
| 3 | Green I | 531 | `gray` | 0.039 – 0.208 |
| 4 | Green | 565 | `gray` | 0.045 – 0.230 |
| 5 | Yellow | 610 | `gray` | 0.040 – 0.257 |
| 6 | Red | 665 | `gray` | 0.029 – 0.253 |
| 7 | Red Edge | 705 | `magma` | 0.045 – 0.278 |
| 8 | NIR | 865 | `inferno` | 0.078 – 0.408 |
...
|---|---|---|
| 1 | 1.9142262775020003e-05 | 0.01 |
| 2 | 1.848685078866485e-05 | 0.01 |
| 3 | 1.9758174064162243e-05 | 0.01 |
| 4 | 2.0050425537932534e-05 | 0.01 |
| 5 | 2.1549499916545107e-05 | 0.01 |
| 6 | 2.4133455321881226e-05 | 0.01 |
| 7 | 2.580015627901804e-05 | 0.01 |
| 8 | 3.8197191698121183e-05 | 0.01 |
...
| `derived/ndvi.png / ndvi_annotated.png` | NDVI = (NIR-Red)/(NIR+Red) (vegetation vigour / greenness); range [-1,1], `RdYlGn`. Scene median +0.555. |
| `derived/ndre.png / ndre_annotated.png` | NDRE = (NIR-RedEdge)/(NIR+RedEdge) (canopy chlorophyll / N status); range [-1,1], `RdYlGn`. Scene median +0.414. |
| `derived/ndwi.png / ndwi_annotated.png` | NDWI = (Green-NIR)/(Green+NIR) (open water (McFeeters)); range [-1,1], `RdBu`. Scene median -0.536. |
| `derived/ndwire.png / ndwire_annotated.png` | NDWIre = (RedEdge-NIR)/(RedEdge+NIR) (red-edge water variant; separates dark water from shadow/wet vegetation); range [-1,1], `RdBu`. Scene median -0.414. |
...
- **RGB / false-colour exposure:** independent per-channel percentile stretch at **2–98%** of valid pixels (a gray-world white balance), then **gamma 1.8**. Per-channel reflectance windows used: Red 0.029–0.253, Green 0.045–0.230, Blue 0.026–0.181.
...
```bash
python visualize_psscene.py Florida_Hydrology_Min_Precipitation_3 \
...
python visualize_psscene.py Florida_Hydrology_Min_Precipitation_3 --s2-fusion
```
