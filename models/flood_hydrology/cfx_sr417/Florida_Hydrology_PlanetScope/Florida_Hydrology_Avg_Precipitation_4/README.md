...
_Scene `20231223_151608_73_24ab` · generated 2026-06-26 00:53 UTC by `visualize_psscene.py`_
...
| Property | Value |
|---|---|
| Item ID | `20231223_151608_73_24ab` |
| Instrument / satellite | PSB.SD / 24ab |
| Acquired (UTC) | 2023-12-23T15:16:08.730097Z |
| Ground sample distance | 3.8 m (pixel grid 3 m) |
| Raster size | 670 × 670 px, 8 bands, uint16 |
| CRS | EPSG:32617 |
| Bounds (CRS units) | (456567.0, 3136929.0, 458577.0, 3138939.0) |
| Sun elev / azim | 29.8° / 146° |
| View angle | 1.1° |
| Cloud / clear | 0% / 100% |
| Quality category | standard |
| UDM2 clear (in-footprint) | 100.0% |
| UDM2 mean confidence | 97.6% |
| Open water (sub-pixel) | 5.0% (20.0 ha; 165 bodies) |

## 2. Original data & paths

| Role | File |
|---|---|
| Surface-reflectance cube (8-band) | `PSScene/20231223_151608_73_24ab_3B_AnalyticMS_SR_8b_clip.tif` |
| UDM2 quality mask (8-band) | `PSScene/20231223_151608_73_24ab_3B_udm2_clip.tif` |
| Band radiometry XML | `PSScene/20231223_151608_73_24ab_3B_AnalyticMS_8b_metadata_clip.xml` |
| Item metadata (STAC props) | `PSScene/20231223_151608_73_24ab_metadata.json` |
...
|---|---|---|---|---|
| 1 | Coastal Blue | 443 | `cividis` | 0.019 – 0.133 |
| 2 | Blue | 490 | `gray` | 0.016 – 0.141 |
| 3 | Green I | 531 | `gray` | 0.022 – 0.162 |
| 4 | Green | 565 | `gray` | 0.025 – 0.175 |
| 5 | Yellow | 610 | `gray` | 0.026 – 0.195 |
| 6 | Red | 665 | `gray` | 0.022 – 0.194 |
| 7 | Red Edge | 705 | `magma` | 0.028 – 0.209 |
| 8 | NIR | 865 | `inferno` | 0.042 – 0.351 |
...
|---|---|---|
| 1 | 3.216179140681572e-05 | 0.01 |
| 2 | 3.10606037448122e-05 | 0.01 |
| 3 | 3.319661214035749e-05 | 0.01 |
| 4 | 3.3687637211332925e-05 | 0.01 |
| 5 | 3.6206300654358884e-05 | 0.01 |
| 6 | 4.0547722341422066e-05 | 0.01 |
| 7 | 4.334802286759215e-05 | 0.01 |
| 8 | 6.417684921368146e-05 | 0.01 |
...
| `derived/ndvi.png / ndvi_annotated.png` | NDVI = (NIR-Red)/(NIR+Red) (vegetation vigour / greenness); range [-1,1], `RdYlGn`. Scene median +0.536. |
| `derived/ndre.png / ndre_annotated.png` | NDRE = (NIR-RedEdge)/(NIR+RedEdge) (canopy chlorophyll / N status); range [-1,1], `RdYlGn`. Scene median +0.384. |
| `derived/ndwi.png / ndwi_annotated.png` | NDWI = (Green-NIR)/(Green+NIR) (open water (McFeeters)); range [-1,1], `RdBu`. Scene median -0.548. |
| `derived/ndwire.png / ndwire_annotated.png` | NDWIre = (RedEdge-NIR)/(RedEdge+NIR) (red-edge water variant; separates dark water from shadow/wet vegetation); range [-1,1], `RdBu`. Scene median -0.384. |
...
- **RGB / false-colour exposure:** independent per-channel percentile stretch at **2–98%** of valid pixels (a gray-world white balance), then **gamma 1.8**. Per-channel reflectance windows used: Red 0.022–0.194, Green 0.025–0.175, Blue 0.016–0.141.
...
```bash
python visualize_psscene.py Florida_Hydrology_Avg_Precipitation_4 \
...
python visualize_psscene.py Florida_Hydrology_Avg_Precipitation_4 --s2-fusion
```
