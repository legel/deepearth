...
_Scene `20240630_162008_62_2475` · generated 2026-06-26 00:54 UTC by `visualize_psscene.py`_
...
| Property | Value |
|---|---|
| Item ID | `20240630_162008_62_2475` |
| Instrument / satellite | PSB.SD / 2475 |
| Acquired (UTC) | 2024-06-30T16:20:08.626752Z |
| Ground sample distance | 3.2 m (pixel grid 3 m) |
| Raster size | 670 × 670 px, 8 bands, uint16 |
| CRS | EPSG:32617 |
| Bounds (CRS units) | (456567.0, 3136929.0, 458577.0, 3138939.0) |
| Sun elev / azim | 73.7° / 104.9° |
| View angle | 5° |
| Cloud / clear | 3% / 23% |
| Quality category | standard |
| UDM2 clear (in-footprint) | 23.0% |
| UDM2 mean confidence | 65.5% |
| Open water (sub-pixel) | 2.9% (11.6 ha; 91 bodies) |

## 2. Original data & paths

| Role | File |
|---|---|
| Surface-reflectance cube (8-band) | `PSScene/20240630_162008_62_2475_3B_AnalyticMS_SR_8b_clip.tif` |
| UDM2 quality mask (8-band) | `PSScene/20240630_162008_62_2475_3B_udm2_clip.tif` |
| Band radiometry XML | `PSScene/20240630_162008_62_2475_3B_AnalyticMS_8b_metadata_clip.xml` |
| Item metadata (STAC props) | `PSScene/20240630_162008_62_2475_metadata.json` |
...
|---|---|---|---|---|
| 1 | Coastal Blue | 443 | `cividis` | 0.029 – 0.214 |
| 2 | Blue | 490 | `gray` | 0.027 – 0.213 |
| 3 | Green I | 531 | `gray` | 0.040 – 0.235 |
| 4 | Green | 565 | `gray` | 0.044 – 0.245 |
| 5 | Yellow | 610 | `gray` | 0.039 – 0.258 |
| 6 | Red | 665 | `gray` | 0.032 – 0.252 |
| 7 | Red Edge | 705 | `magma` | 0.060 – 0.280 |
| 8 | NIR | 865 | `inferno` | 0.130 – 0.448 |
...
|---|---|---|
| 1 | 1.7790555322519083e-05 | 0.01 |
| 2 | 1.7181424451245622e-05 | 0.01 |
| 3 | 1.836297478995781e-05 | 0.01 |
| 4 | 1.8634589283672885e-05 | 0.01 |
| 5 | 2.0027808360159527e-05 | 0.01 |
| 6 | 2.2429300917744913e-05 | 0.01 |
| 7 | 2.3978309827116504e-05 | 0.01 |
| 8 | 3.549994376616123e-05 | 0.01 |
...
| `derived/ndvi.png / ndvi_annotated.png` | NDVI = (NIR-Red)/(NIR+Red) (vegetation vigour / greenness); range [-1,1], `RdYlGn`. Scene median +0.430. |
| `derived/ndre.png / ndre_annotated.png` | NDRE = (NIR-RedEdge)/(NIR+RedEdge) (canopy chlorophyll / N status); range [-1,1], `RdYlGn`. Scene median +0.328. |
| `derived/ndwi.png / ndwi_annotated.png` | NDWI = (Green-NIR)/(Green+NIR) (open water (McFeeters)); range [-1,1], `RdBu`. Scene median -0.415. |
| `derived/ndwire.png / ndwire_annotated.png` | NDWIre = (RedEdge-NIR)/(RedEdge+NIR) (red-edge water variant; separates dark water from shadow/wet vegetation); range [-1,1], `RdBu`. Scene median -0.328. |
...
- **RGB / false-colour exposure:** independent per-channel percentile stretch at **2–98%** of valid pixels (a gray-world white balance), then **gamma 1.8**. Per-channel reflectance windows used: Red 0.032–0.252, Green 0.044–0.245, Blue 0.027–0.213.
...
```bash
python visualize_psscene.py Florida_Hydrology_Max_Precipitation_3 \
...
python visualize_psscene.py Florida_Hydrology_Max_Precipitation_3 --s2-fusion
```
