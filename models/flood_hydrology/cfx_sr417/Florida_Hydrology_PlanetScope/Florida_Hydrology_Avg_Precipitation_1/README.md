...
_Scene `20250617_162644_46_2521` · generated 2026-06-26 00:46 UTC by `visualize_psscene.py`_
...
| Property | Value |
|---|---|
| Item ID | `20250617_162644_46_2521` |
| Instrument / satellite | PSB.SD / 2521 |
| Acquired (UTC) | 2025-06-17T16:26:44.468907Z |
| Ground sample distance | 3.7 m (pixel grid 3 m) |
| Raster size | 670 × 670 px, 8 bands, uint16 |
| CRS | EPSG:32617 |
| Bounds (CRS units) | (456567.0, 3136929.0, 458577.0, 3138939.0) |
| Sun elev / azim | 75.3° / 106.4° |
| View angle | 4.6° |
| Cloud / clear | 0% / 100% |
| Quality category | standard |
| UDM2 clear (in-footprint) | 100.0% |
| UDM2 mean confidence | 97.3% |
| Open water (sub-pixel) | 1.8% (7.3 ha; 225 bodies) |

## 2. Original data & paths

| Role | File |
|---|---|
| Surface-reflectance cube (8-band) | `PSScene/20250617_162644_46_2521_3B_AnalyticMS_SR_8b_clip.tif` |
| UDM2 quality mask (8-band) | `PSScene/20250617_162644_46_2521_3B_udm2_clip.tif` |
| Band radiometry XML | `PSScene/20250617_162644_46_2521_3B_AnalyticMS_8b_metadata_clip.xml` |
| Item metadata (STAC props) | `PSScene/20250617_162644_46_2521_metadata.json` |
...
|---|---|---|---|---|
| 1 | Coastal Blue | 443 | `cividis` | 0.013 – 0.253 |
| 2 | Blue | 490 | `gray` | 0.019 – 0.279 |
| 3 | Green I | 531 | `gray` | 0.037 – 0.317 |
| 4 | Green | 565 | `gray` | 0.040 – 0.321 |
| 5 | Yellow | 610 | `gray` | 0.034 – 0.334 |
| 6 | Red | 665 | `gray` | 0.026 – 0.342 |
| 7 | Red Edge | 705 | `magma` | 0.064 – 0.349 |
| 8 | NIR | 865 | `inferno` | 0.123 – 0.441 |
...
|---|---|---|
| 1 | 1.7637555103057407e-05 | 0.01 |
| 2 | 1.7033662806707337e-05 | 0.01 |
| 3 | 1.820505171662482e-05 | 0.01 |
| 4 | 1.847433030365267e-05 | 0.01 |
| 5 | 1.9855567583023013e-05 | 0.01 |
| 6 | 2.223640710973415e-05 | 0.01 |
| 7 | 2.377209441678442e-05 | 0.01 |
| 8 | 3.519464136898285e-05 | 0.01 |
...
| `derived/ndvi.png / ndvi_annotated.png` | NDVI = (NIR-Red)/(NIR+Red) (vegetation vigour / greenness); range [-1,1], `RdYlGn`. Scene median +0.552. |
| `derived/ndre.png / ndre_annotated.png` | NDRE = (NIR-RedEdge)/(NIR+RedEdge) (canopy chlorophyll / N status); range [-1,1], `RdYlGn`. Scene median +0.403. |
| `derived/ndwi.png / ndwi_annotated.png` | NDWI = (Green-NIR)/(Green+NIR) (open water (McFeeters)); range [-1,1], `RdBu`. Scene median -0.545. |
| `derived/ndwire.png / ndwire_annotated.png` | NDWIre = (RedEdge-NIR)/(RedEdge+NIR) (red-edge water variant; separates dark water from shadow/wet vegetation); range [-1,1], `RdBu`. Scene median -0.403. |
...
- **RGB / false-colour exposure:** independent per-channel percentile stretch at **2–98%** of valid pixels (a gray-world white balance), then **gamma 1.8**. Per-channel reflectance windows used: Red 0.026–0.342, Green 0.040–0.321, Blue 0.019–0.279.
...
```bash
python visualize_psscene.py Florida_Hydrology_Avg_Precipitation_1 \
...
python visualize_psscene.py Florida_Hydrology_Avg_Precipitation_1 --s2-fusion
```
