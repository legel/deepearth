...
\IfFontExistsTF{Menlo}{\newfontfamily\ghcodefont{Menlo}[Scale=0.9]}
  {\newfontfamily\ghcodefont{Courier New}[Scale=0.9]}

% Prefer the original Oxygen family, with a portable Arial fallback.
\IfFontExistsTF{Oxygen}{
  \setmainfont{Oxygen}[Ligatures={NoRequired,NoCommon,NoContextual}]
  \newfontfamily\oxygenlight{Oxygen Light}[Ligatures={NoRequired,NoCommon,NoContextual}]
  \newfontfamily\oxygenbold{Oxygen}[BoldFont=Oxygen Bold,Ligatures={NoRequired,NoCommon,NoContextual}]
}{
  \setmainfont{Arial}
  \newfontfamily\oxygenlight{Arial}
  \newfontfamily\oxygenbold{Arial}[BoldFont=Arial Bold]
}
...
\item DeepEarth fuses multi-modal ($e.g.$ vision-language) and space-time embeddings
\item Model trains by masked reconstruction of multi-modal data across space-time
\end{itemize}
\vspace{1mm}
}
...
%-----------------------------------------------------------------------
\block{Historical Exploratory Ecological Benchmark}{
\vspace{1mm}
...
{\oxygenbold Historical result:} A development run reported lower error than a pre-trained Vision Transformer with less input data. This comparison is hypothesis-generating, not confirmatory.
...
\vspace{10mm}
\raggedright
{\oxygenbold Evidence status:} These historical figures used test-set feedback during development and did not evaluate causal future rollout; they do not establish state of the art. Only coordinates and species names were used.
\vspace{1mm}
}
...
\vspace{3mm}
{\oxygenbold Historical exploratory figures:} 33\% fewer collisions; reported MAE 27\% lower and R² 30\% higher. Confirmation is pending.
\vspace{1mm}
}
...
\item Xu et al. {\oxygenbold ``Grid4D: 4D Decomposed Hash Encoding for High-Fidelity Dynamic Gaussian Splatting.''} {\itshape NeurIPS}, 2024.
\item Yebra et al. {\oxygenbold ``Globe-LFMC 2.0, an enhanced and updated dataset for live fuel moisture content research.''} {\itshape Scientif{}ic Data}, 2024.
