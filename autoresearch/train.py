...
def train_and_evaluate(config, device):
    ...
    model = DeepEarth(variables, d_model=m.get("d_model", 256), n_latents=m.get("n_latents", 24),
                      ...
                      read_depth=m.get("read_depth", 1), read_op=m.get("read_op", "mha"), neighbor_op=m.get("neighbor_op", "add"), token_op=m.get("token_op", "add"), detail_evidence_tokens=m.get("detail_evidence_tokens", 0), read_cond=m.get("read_cond", False), joint_decode=m.get("joint_decode", False), grad_checkpoint=m.get("grad_checkpoint", False), diffusion=m.get("diffusion", False), loss_weights=m.get("loss_weights"),
