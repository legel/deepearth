...
| | |
|---|---|
| **Surface** | `encoders/spacetime/earth4d.py` + `autoresearch/spacetime/` probes + the channels they feed. Never the fusion model. |
| **Shared state** | **Ensue** (swarm-wide) ⇄ `records.json` (per-run ledger) ⇄ `scorecard.md` (the board) |
| **Ops** | box, GPUs, token, commit identity → `autoresearch/spacetime/program/box-operations.md` |
