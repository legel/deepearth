...
Every successful run produces the same trace:
  ...
  - RECORD tracking in autoresearch/spacetime/state/records.json, and one upserted Ensue key per capability.
A failed probe (rc != 0) writes no trace and no record; the log is kept for diagnosis.

Usage:
  python -m deepearth.autoresearch.spacetime.harness.trace --metric family_from_spacetime \
...
sys.path.insert(0, str(Path(__file__).resolve().parents[5]))  # dir holding the deepearth package
from deepearth.autoresearch.spacetime.harness.probe_contract import (  # noqa: E402
    ContractError,
    ProbeResult,
)
from deepearth.autoresearch.spacetime.harness import probe_registry  # noqa: E402
...
def main() -> None:
    ...
    log_path = a.log or str(Path(__file__).resolve().parents[1] / "state" / "traces" / f"{tag}.log")
