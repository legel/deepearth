"""autoresearch/spacetime/editable_files/harness/trace.py — the Earth4D probe harness and record ledger.

The surface is the spacetime encoder (encoders/spacetime/earth4d.py) and the data channels feeding it,
measured by the fast encoder probe (autoresearch/spacetime/editable_files/harness/probe.py & friends) — NOT full-model
...
Every successful run produces the same trace:
  ...
  - RECORD tracking in autoresearch/spacetime/records/records.json, and one upserted Ensue key per capability.
A failed probe (rc != 0) writes no trace and no record; the log is kept for diagnosis.

Usage:
  python -m deepearth.autoresearch.spacetime.editable_files.harness.trace --metric family_from_spacetime \
...
sys.path.insert(0, str(Path(__file__).resolve().parents[6]))  # dir holding the deepearth package
from deepearth.autoresearch.spacetime.editable_files.harness.probe_contract import (  # noqa: E402
    ContractError,
    ProbeResult,
)
...
RECORDS = Path(__file__).resolve().parents[2] / "records" / "records.json"  # the machine record (fill scorecard by breaking these)
...
ENSUE_ENV_FILE = Path(__file__).resolve().parents[4] / ".env"   # autoresearch/.env, gitignored
...
def main() -> None:
    ...
    log_path = a.log or str(Path(__file__).resolve().parents[2] / "records" / "traces" / f"{tag}.log")
