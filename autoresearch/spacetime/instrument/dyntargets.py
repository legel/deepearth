...
def run_first_arrival_all(e4d, rff, raw, fd, days, coords_ll, sp, test, dev, feats=("e4d", "rff", "raw"), **kw):
    """feats: subset of (e4d,rff,raw) to TRAIN (HARD RULE one feat/head per run); skipped return NaN dicts."""
    src = {"e4d": e4d, "rff": rff, "raw": raw}
    r = {ft: (run_first_arrival(src[ft], fd[ft], days, coords_ll, sp, test, dev, **kw)
              if ft in feats else _nan_dyn()) for ft in ("e4d", "rff", "raw")}
    n_te = next((r[ft]["n_te"] for ft in feats if r[ft]["n_te"]), 0)
...
def run_abundance(qfeat_all, feat_dim, days, coords_ll, test, dev,
                  K=16, steps=4000, lr=3e-3, hidden=256, hops=2, win=90.0, lead=0.0, delta=False):
    lat = coords_ll[:, 0].numpy(); lon = coords_ll[:, 1].numpy()
    tgt = _abundance_target(lat, lon, days, win=win, lead=lead, delta=delta)
...
def run_abundance_all(e4d, rff, raw, fd, days, coords_ll, test, dev, feats=("e4d", "rff", "raw"), **kw):
    """feats: subset of (e4d,rff,raw) to TRAIN (HARD RULE one feat/head per run); skipped return NaN dicts."""
    src = {"e4d": e4d, "rff": rff, "raw": raw}
    r = {ft: (run_abundance(src[ft], fd[ft], days, coords_ll, test, dev, **kw)
              if ft in feats else _nan_dyn()) for ft in ("e4d", "rff", "raw")}
    n_te = next((r[ft]["n_te"] for ft in feats if r[ft]["n_te"]), 0)
    for ft in ("e4d", "rff", "raw"):
        if ft not in feats:
...
class StaticVecEnv(nn.Module):
    """ENV-ONLY floor: predict (cos,sin) DOY from the query's OWN space-feature ++ its env covariates."""

    def __init__(self, feat_dim, env_dim, hidden=256):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(feat_dim + env_dim, hidden), nn.GELU(),
                                 nn.Linear(hidden, hidden), nn.GELU(), nn.Linear(hidden, 2))

    def forward(self, qfeat, qenv):
        return F.normalize(self.net(torch.cat([qfeat, qenv], -1)), dim=-1)


class LSTMVecEnv(nn.Module):
    """ENV-CONDITIONED LSTM propagator: each neighbour step carries its OWN env (the climate that entrained its
    timing) alongside its DOY; the query's OWN env is fused before the head. use_nenv toggles the neighbour-env
    channel for the neighbour+env vs neighbour-only ablation. Leak-guards intact (spatial edge, space-only q)."""

    def __init__(self, feat_dim, env_dim, hidden=256, use_nenv=True):
        super().__init__()
        self.use_nenv = use_nenv
        self.lstm = nn.LSTM(feat_dim + 2 + 2 + (env_dim if use_nenv else 0), hidden, batch_first=True)
        self.head = nn.Sequential(nn.Linear(hidden + env_dim, hidden), nn.GELU(), nn.Linear(hidden, 2))

    def forward(self, nfeat, ndoy, edge, nenv, qenv, lengths):
        parts = [nfeat, ndoy, edge] + ([nenv] if self.use_nenv else [])
        x = torch.cat(parts, -1)
        packed = nn.utils.rnn.pack_padded_sequence(x, lengths.cpu().clamp(min=1),
                                                   batch_first=True, enforce_sorted=False)
        _, (h, _) = self.lstm(packed)
        return F.normalize(self.head(torch.cat([h[-1], qenv], -1)), dim=-1)
