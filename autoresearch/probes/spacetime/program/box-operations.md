...
- Run one deliberate swing at a time:
  `cd /workspace/deepearth && PYTHONPATH=/workspace python3.12 -m deepearth.autoresearch.probes.spacetime.editable_files.harness --metric <cap> --probe "<flags>" --tag <id> --device cuda:N --ensue`
...
## Ensue + commits
- **Ensue token:** `/workspace/.env` (`ENSUE_API_TOKEN`); `trace.py` reads it automatically. **NEVER commit it.**
