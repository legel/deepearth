...
def write_scorecard(recs: dict, path: Path = SCORECARD_TXT) -> Path:
    ...
    lines += [
        ...
        "",
        "SHARE IS CONFOUNDED WITH ENCODER OUTPUT WIDTH — measured, not suspected.  probe.py builds the",
        "       control as fair_rff(rn, e4d.shape[1]), so the baseline is DIMENSION-MATCHED to the encoder,",
        "       and RFF accuracy is non-monotone in width. Padding the encoder output with columns of",
        "       LITERAL ZEROS — adding no information at all — moved family_from_spacetime's share from",
        "       20.7% (dim 2592) to 27.2% (dim 3024) and down to 15.1% (dim 3744). This legend called the",
        "       zero-padded encoder EARNING.",
        "       So: compare share ONLY between arms of EQUAL output dim, and treat a share change from any",
        "       arm that alters width as an artifact until a same-width zero-pad null fails to reproduce it.",
        "       Fixing this means fitting the control at a FIXED dimension, which re-baselines every row —",
        "       an operator decision, not an agent's.",
        "",
