...
from the comparison. Six modes are in that category, which is not obvious from their output.

Usage:
    python -m deepearth.autoresearch.programs.spacetime.probe_registry --capability family_from_env
    python -m deepearth.autoresearch.programs.spacetime.probe_registry --all
"""
...
MODES: Tuple[Mode, ...] = (
    ...
    Mode("PHENO-DENSEFIELD(mean-DOY, same-cell-EXCLUDED)", "--pheno_densefield",
         ...
         reason="runs on RAW spatial features only; Earth4D is not in the comparison"),
    Mode("BREADTH(<target>)", "--breadth_target {occupancy,richness,community_activity}",
         requires=("--forecast",), records=False, lever=ARCH,
         reason="target is not a scorecard capability, and it evaluates on raw PE only"),
    Mode("PROPAGATOR-ARCH(<target>)", "--abund_prop_arch [--prop_arch lstm1,lstm2,lstm3,attn]",
         requires=("--forecast",), records=False, lever=ARCH,
         reason="propagator-architecture comparison on raw PE only"),
    Mode("FIRST-ARRIVAL(onset-DOY)", "--first_arrival", requires=("--forecast",), records=False,
         lever=ARCH, reason="target is not a scorecard capability, and it evaluates on raw PE only"),
    Mode("ABUNDANCE(<target>)", "--abundance [--abund_win D] [--abund_delta]",
         requires=("--forecast",), records=False, lever=ARCH,
         reason="target is not a scorecard capability, and it evaluates on raw PE only"),

    # ---- quarantined -----------------------------------------------------------------------------
    Mode("AR-ROLLOUT(<target>)", "--ar_rollout  [QUARANTINED]", requires=("--forecast",),
         records=False, lever=ARCH,
         reason="QUARANTINED: neighbour state or target window can cross the forecast origin. "
                "Blocked until future-sentinel, horizon-purge and right-censoring tests pass."),
    Mode("CONTINUOUS-LEAD(community-activity)", "--ar_cond_lead  [QUARANTINED]",
         requires=("--forecast",), records=False, lever=ARCH,
         reason="QUARANTINED: same origin-crossing risk as --ar_rollout."),
)
...
LEVER_SITES = {
    DATA: [
        "autoresearch/programs/spacetime/probe.py: load_env / load_vision / load_env_species "
    ...
    ARCH: [
        ...
        "autoresearch/programs/spacetime/recurrence.py: run_recurrence, run_field_decode, propagators",
        "autoresearch/programs/spacetime/gnn.py: message passing",
