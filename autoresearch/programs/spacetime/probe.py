...
from deepearth.autoresearch.programs.spacetime import probe_modes_dynamics, probe_modes_tables
...
def main(argv=None):
    ...
    # Dynamics / AR modes: all diagnostics, none able to set a record. See probe_modes_dynamics.py.
    _dyn_ctx = _DynamicsCtx(a=a, dev=dev, t0=t0, lat=lat, lon=lon, days=days, test=test,
                            obs_index=obs_index, load_species=load_species,
                            enc=enc, e4d=e4d)
    for _flag, _mode in (
        (a.ar_rollout, probe_modes_dynamics.ar_rollout_mode),
        (a.ar_cond_lead, probe_modes_dynamics.ar_cond_lead_mode),
        (a.breadth_target, probe_modes_dynamics.breadth_mode),
        (a.abund_prop_arch, probe_modes_dynamics.prop_arch_mode),
        (a.first_arrival or a.abundance, probe_modes_dynamics.arrival_abundance_mode),
    ):
        if _flag:
            return _mode(_dyn_ctx)
