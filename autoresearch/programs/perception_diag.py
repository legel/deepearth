...
def probe_genus_from_semantic(emb, bino, min_species=4, seed=0):
    ...
    def cv_acc(labels):
        ...
        accs = []
        for tr, te in skf.split(Xs, labels):
            clf = LogisticRegression(max_iter=400, C=1.0, multi_class="multinomial")
            clf.fit(Xs[tr], labels[tr])
            accs.append((clf.predict(Xs[te]) == labels[te]).mean())
        return float(np.mean(accs))

    real = cv_acc(y)
    yl = y.copy(); rng.shuffle(yl)
    null = cv_acc(yl)
    return dict(n_species=int(m.sum()), n_genera=len(classes),
